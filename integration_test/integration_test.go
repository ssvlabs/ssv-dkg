...
func TestUnhappyFlows(t *testing.T) {
	...
	t.Run("test wrong operator amount 5,6,8,9,11,12", func(t *testing.T) {
		...
		_, _, err = clnt.StartReshare(crypto.NewID(), []uint64{1, 2, 3}, marshalledKs, cSigs)
		require.ErrorContains(t, err, "wrong operators len: < 4")
		_, _, err = clnt.StartReshare(crypto.NewID(), []uint64{5, 6, 7}, marshalledKs, cSigs)
		require.ErrorContains(t, err, "wrong operators len: < 4")
		_, _, err = clnt.StartReshare(crypto.NewID(), []uint64{5, 6, 7, 8, 9}, marshalledKs, cSigs)
		require.ErrorContains(t, err, "amount of operators should be 4,7,10,13")
		_, _, err = clnt.StartReshare(crypto.NewID(), []uint64{5, 6, 7, 8, 9, 10, 11, 12}, marshalledKs, cSigs)
		require.ErrorContains(t, err, "amount of operators should be 4,7,10,13")
		_, _, err = clnt.StartReshare(crypto.NewID(), []uint64{5, 6, 7, 8, 9, 10, 11, 12, 13}, marshalledKs, cSigs)
		require.ErrorContains(t, err, "amount of operators should be 4,7,10,13")
	})
	srv1.HttpSrv.Close()
	srv2.HttpSrv.Close()
