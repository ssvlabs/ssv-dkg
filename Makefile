...
install:
	$(GOINSTALL) -ldflags "-X main.Version=`git describe --tags --abbrev=0`" cmd/ssv-dkg/ssv-dkg.go
	@echo "Done building."
...
build:
	...
	go build -o $(BINARY_NAME) -ldflags "-X main.Version=`git describe --tags --abbrev=0`" ./cmd/ssv-dkg/ssv-dkg.go
