...
openssl req -x509 -new -key ./initiator/rootCA.key -subj "/C=CN/ST=GD/L=SZ/O=ssv, Inc./CN=*ssvlabs.io" -days 3650 -out ./initiator/rootCA.crt


for i in $(seq 1 8);
do
    openssl genrsa -out ./operator${i}/operator${i}.key 2048 && \
    openssl req -newkey rsa:2048 -nodes -keyout ./operator${i}/operator${i}.key -subj "/C=CN/ST=GD/L=SZ/O=ssv, Inc./CN=*ssvlabs.io" -out ./operator${i}/operator${i}.csr && \
