...
// Hash: e80ac70a7ae97511f0177d561da0249282ee00be856a8a0e9b805b5d3213f74e
// Version: 0.1.3
package wire
...
// MarshalSSZ ssz marshals the BulkReshareMessage object
func (b *BulkReshareMessage) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BulkReshareMessage object to a target array
func (b *BulkReshareMessage) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(4)

	// Offset (0) 'Reshares'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(b.Reshares); ii++ {
		offset += 4
		offset += b.Reshares[ii].SizeSSZ()
	}

	// Field (0) 'Reshares'
	if size := len(b.Reshares); size > 100 {
		err = ssz.ErrListTooBigFn("BulkReshareMessage.Reshares", size, 100)
		return
	}
	{
		offset = 4 * len(b.Reshares)
		for ii := 0; ii < len(b.Reshares); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += b.Reshares[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(b.Reshares); ii++ {
		if dst, err = b.Reshares[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}
...
// UnmarshalSSZ ssz unmarshals the BulkReshareMessage object
func (b *BulkReshareMessage) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 4 {
		return ssz.ErrSize
	...
	// Field (0) 'Reshares'
	{
		...
		b.Reshares = make([]*ReshareMessage, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if b.Reshares[indx] == nil {
				b.Reshares[indx] = new(ReshareMessage)
			}
			if err = b.Reshares[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
...
// SizeSSZ returns the ssz encoded size in bytes for the BulkReshareMessage object
func (b *BulkReshareMessage) SizeSSZ() (size int) {
	size = 4

	// Field (0) 'Reshares'
	for ii := 0; ii < len(b.Reshares); ii++ {
		size += 4
		size += b.Reshares[ii].SizeSSZ()
	}

	return
}

// HashTreeRoot ssz hashes the BulkReshareMessage object
func (b *BulkReshareMessage) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BulkReshareMessage object with a hasher
func (b *BulkReshareMessage) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()

	// Field (0) 'Reshares'
	{
		subIndx := hh.Index()
		num := uint64(len(b.Reshares))
		if num > 100 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range b.Reshares {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
...
// GetTree ssz hashes the BulkReshareMessage object
func (b *BulkReshareMessage) GetTree() (*ssz.Node, error) {
	return ssz.ProofTree(b)
}

// MarshalSSZ ssz marshals the SignatureForHash object
func (s *SignatureForHash) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(s)
}

// MarshalSSZTo ssz marshals the SignatureForHash object to a target array
func (s *SignatureForHash) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	offset := int(8)
	...
	// Field (0) 'Hash'
	if size := len(s.Hash); size > 32 {
		err = ssz.ErrBytesLengthFn("SignatureForHash.Hash", size, 32)
		return
	}
	...
	// Field (1) 'Signature'
	if size := len(s.Signature); size > 2048 {
		err = ssz.ErrBytesLengthFn("SignatureForHash.Signature", size, 2048)
		return
	}
	dst = append(dst, s.Signature...)
...
// UnmarshalSSZ ssz unmarshals the SignatureForHash object
func (s *SignatureForHash) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size < 8 {
		return ssz.ErrSize
	...
	// Field (0) 'Hash'
	{
		...
		if cap(s.Hash) == 0 {
			s.Hash = make([]byte, 0, len(buf))
		}
		s.Hash = append(s.Hash, buf...)
	}
...
// SizeSSZ returns the ssz encoded size in bytes for the SignatureForHash object
func (s *SignatureForHash) SizeSSZ() (size int) {
	size = 8
...
// HashTreeRoot ssz hashes the SignatureForHash object
func (s *SignatureForHash) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(s)
}

// HashTreeRootWith ssz hashes the SignatureForHash object with a hasher
func (s *SignatureForHash) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()

	// Field (0) 'Hash'
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(s.Hash))
		if byteLen > 32 {
			err = ssz.ErrIncorrectListSize
			return
		}
...
// GetTree ssz hashes the SignatureForHash object
func (s *SignatureForHash) GetTree() (*ssz.Node, error) {
	return ssz.ProofTree(s)
}
