...
// Hash: c4830fb66db645385dc993ee28ad21d76d1f56ded9ad7660395c52be952793f0
// Version: 0.1.3
package wire
...
func (r *Reshare) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (0) 'ValidatorPub'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.ValidatorPub)
	...
	// Offset (8) 'CeremonySigs'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.CeremonySigs)

	// Offset (9) 'InitiatorPublicKey'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.InitiatorPublicKey)

	// Field (0) 'ValidatorPub'
	if size := len(r.ValidatorPub); size > 48 {
		err = ssz.ErrBytesLengthFn("Reshare.ValidatorPub", size, 48)
		return
	}
	dst = append(dst, r.ValidatorPub...)
...
func (r *Reshare) UnmarshalSSZ(buf []byte) error {
	...
	// Field (3) 'OldT'
	r.OldT = ssz.UnmarshallUint64(buf[12:20])

	// Field (4) 'NewT'
	r.NewT = ssz.UnmarshallUint64(buf[20:28])

	// Field (5) 'Owner'
	copy(r.Owner[:], buf[28:48])

	// Field (6) 'Nonce'
	r.Nonce = ssz.UnmarshallUint64(buf[48:56])

	// Offset (7) 'Keyshares'
	if o7 = ssz.ReadOffset(buf[56:60]); o7 > size || o2 > o7 {
		return ssz.ErrOffset
	}
	...
	// Offset (9) 'InitiatorPublicKey'
	if o9 = ssz.ReadOffset(buf[64:68]); o9 > size || o8 > o9 {
		return ssz.ErrOffset
	}
	...
	// Field (0) 'ValidatorPub'
	{
		...
		if cap(r.ValidatorPub) == 0 {
			r.ValidatorPub = make([]byte, 0, len(buf))
		}
		r.ValidatorPub = append(r.ValidatorPub, buf...)
	}
...
func (r *Reshare) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(r.ValidatorPub))
		if byteLen > 48 {
			err = ssz.ErrIncorrectListSize
			return
		}
		hh.Append(r.ValidatorPub)
