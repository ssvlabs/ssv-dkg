...
// Hash: eb0e83caf3a4220e7e37451973a1cc37c7eb35940bd872c2ced3c5ce1fdc04c5
// Version: 0.1.3
package wire
...
func (i *Init) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (7) 'NewOperators'
	dst = ssz.WriteOffset(dst, offset)
	for ii := 0; ii < len(i.NewOperators); ii++ {
		offset += 4
		offset += i.NewOperators[ii].SizeSSZ()
	}

	// Field (8) 'NewT'
	dst = ssz.MarshalUint64(dst, i.NewT)

	// Field (9) 'OldID'
	dst = append(dst, i.OldID[:]...)
	...
	// Field (7) 'NewOperators'
	if size := len(i.NewOperators); size > 26 {
		err = ssz.ErrListTooBigFn("Init.NewOperators", size, 26)
		return
	}
	{
		offset = 4 * len(i.NewOperators)
		for ii := 0; ii < len(i.NewOperators); ii++ {
			dst = ssz.WriteOffset(dst, offset)
			offset += i.NewOperators[ii].SizeSSZ()
		}
	...
	for ii := 0; ii < len(i.NewOperators); ii++ {
		if dst, err = i.NewOperators[ii].MarshalSSZTo(dst); err != nil {
			return
		}
	}

	// Field (10) 'Coefs'
	if size := len(i.Coefs); size > 4096 {
		err = ssz.ErrBytesLengthFn("Init.Coefs", size, 4096)
		return
	}
	dst = append(dst, i.Coefs...)
...
func (i *Init) UnmarshalSSZ(buf []byte) error {
	...
	// Offset (7) 'NewOperators'
	if o7 = ssz.ReadOffset(buf[52:56]); o7 > size || o6 > o7 {
		return ssz.ErrOffset
	}

	// Field (8) 'NewT'
	i.NewT = ssz.UnmarshallUint64(buf[56:64])

	// Field (9) 'OldID'
	copy(i.OldID[:], buf[64:88])

	// Offset (10) 'Coefs'
	if o10 = ssz.ReadOffset(buf[88:92]); o10 > size || o7 > o10 {
		return ssz.ErrOffset
	}
	...
	{
		buf = tail[o7:o10]
		num, err := ssz.DecodeDynamicLength(buf, 26)
		if err != nil {
			return err
		}
		i.NewOperators = make([]*Operator, num)
		err = ssz.UnmarshalDynamic(buf, num, func(indx int, buf []byte) (err error) {
			if i.NewOperators[indx] == nil {
				i.NewOperators[indx] = new(Operator)
			}
			if err = i.NewOperators[indx].UnmarshalSSZ(buf); err != nil {
				return err
			}
			return nil
		})
	...
	// Field (10) 'Coefs'
	{
		...
		if cap(i.Coefs) == 0 {
			i.Coefs = make([]byte, 0, len(buf))
		}
		i.Coefs = append(i.Coefs, buf...)
	}
	return err
}
...
func (i *Init) SizeSSZ() (size int) {
	...
	for ii := 0; ii < len(i.NewOperators); ii++ {
		size += 4
		size += i.NewOperators[ii].SizeSSZ()
	}
...
func (i *Init) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		subIndx := hh.Index()
		num := uint64(len(i.NewOperators))
		if num > 26 {
			err = ssz.ErrIncorrectListSize
			return
		}
		for _, elem := range i.NewOperators {
			if err = elem.HashTreeRootWith(hh); err != nil {
				return
			}
		}
		hh.MerkleizeWithMixin(subIndx, num, 26)
	}
