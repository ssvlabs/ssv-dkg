...
// Hash: dea95a702693fe2573343e4ce3f8ddaa28b25e3f5bdf42c34d91a3e6de5b12e3
// Version: 0.1.3
package wire
...
func (r *Reshare) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (6) 'Keyshares'
	if size := len(r.Keyshares); size > 8388608 {
		err = ssz.ErrBytesLengthFn("Reshare.Keyshares", size, 8388608)
		return
	}
	dst = append(dst, r.Keyshares...)

	// Field (7) 'CeremonySigs'
	if size := len(r.CeremonySigs); size > 3328 {
		err = ssz.ErrBytesLengthFn("Reshare.CeremonySigs", size, 3328)
		return
	}
...
func (r *Reshare) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	// Field (7) 'CeremonySigs'
	{
		...
		hh.MerkleizeWithMixin(elemIndx, byteLen, (3328+31)/32)
	}
...
func (r *ResultData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (2) 'DepositData'
	if size := len(r.DepositData); size > 8388608 {
		err = ssz.ErrBytesLengthFn("ResultData.DepositData", size, 8388608)
		return
	}
	dst = append(dst, r.DepositData...)

	// Field (3) 'KeysharesData'
	if size := len(r.KeysharesData); size > 8388608 {
		err = ssz.ErrBytesLengthFn("ResultData.KeysharesData", size, 8388608)
		return
	}
