...
// Hash: 4e28d6287763cf2b02a110b629bd6541a199661fff3a6aade29b399303ee156d
// Version: 0.1.3
package wire
...
func (r *ReSign) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (2) 'ExitMessage'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.ExitMessage)
	...
	// Field (2) 'ExitMessage'
	if size := len(r.ExitMessage); size > 32768 {
		err = ssz.ErrBytesLengthFn("ReSign.ExitMessage", size, 32768)
		return
	}
	dst = append(dst, r.ExitMessage...)
...
func (r *ReSign) UnmarshalSSZ(buf []byte) error {
	...
	// Field (2) 'ExitMessage'
	{
		...
		if cap(r.ExitMessage) == 0 {
			r.ExitMessage = make([]byte, 0, len(buf))
		}
		r.ExitMessage = append(r.ExitMessage, buf...)
	}
	return err
}
...
func (r *ReSign) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(r.ExitMessage))
		if byteLen > 32768 {
			err = ssz.ErrIncorrectListSize
			return
		}
		hh.Append(r.ExitMessage)
		hh.MerkleizeWithMixin(elemIndx, byteLen, (32768+31)/32)
	}
...
func (r *ReSignResult) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Offset (1) 'ExitMessagePartialSig'
	dst = ssz.WriteOffset(dst, offset)
	offset += len(r.ExitMessagePartialSig)

	// Field (1) 'ExitMessagePartialSig'
	if size := len(r.ExitMessagePartialSig); size > 96 {
		err = ssz.ErrBytesLengthFn("ReSignResult.ExitMessagePartialSig", size, 96)
		return
	}
	dst = append(dst, r.ExitMessagePartialSig...)
...
func (r *ReSignResult) UnmarshalSSZ(buf []byte) error {
	...
	// Field (1) 'ExitMessagePartialSig'
	{
		...
		if cap(r.ExitMessagePartialSig) == 0 {
			r.ExitMessagePartialSig = make([]byte, 0, len(buf))
		}
		r.ExitMessagePartialSig = append(r.ExitMessagePartialSig, buf...)
	}
	return err
}
...
func (r *ReSignResult) SizeSSZ() (size int) {
	...
	// Field (1) 'ExitMessagePartialSig'
	size += len(r.ExitMessagePartialSig)
...
func (r *ReSignResult) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	...
	{
		elemIndx := hh.Index()
		byteLen := uint64(len(r.ExitMessagePartialSig))
		if byteLen > 96 {
			err = ssz.ErrIncorrectListSize
			return
		}
		hh.Append(r.ExitMessagePartialSig)
