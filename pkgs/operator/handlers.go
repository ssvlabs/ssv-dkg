...
func (s *Server) resignHandler(writer http.ResponseWriter, request *http.Request) {
	...
	writer.WriteHeader(http.StatusOK)
	if _, err := writer.Write(b); err != nil {
		s.Logger.Error("error sending resign response", zap.Error(err))
		return
	}
}
...
func (s *Server) reshareHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if signedReshareMsg.Message.Type != wire.ReshareMessageType {
		utils.WriteErrorResponse(s.Logger, writer, fmt.Errorf("operator %d, err: %w", s.State.OperatorID, errors.New("not reshare message to reshare route")), http.StatusBadRequest)
		return
	}
	...
	writer.WriteHeader(http.StatusOK)
	if _, err := writer.Write(b); err != nil {
		s.Logger.Error("error sending reshare response", zap.Error(err))
		return
	}
}
