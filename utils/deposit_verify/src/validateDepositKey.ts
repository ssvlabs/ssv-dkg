...
import { verifyDepositRoots } from "./SSZ.js";
import { DepositKeyInterface, BeaconchainDepositInterface } from "./index.js";
import {
  ETHER_TO_GWEI,
  BEACONCHAIN_URL,
  MIN_DEPOSIT_AMOUNT,
...
export const validateDepositKey = async (
  ...
  const depositKeysStatuses: boolean[] = files.map((depositDatum) => {
    ...
    {
      console.log("err verifying deposit root");
      return false;
    }
...
export const getExistingDepositsForPubkeys = async (
  ...
  const { data: beaconScanCheck } =
    await axios.get<BeaconchainDepositInterface>(beaconScanUrl);
